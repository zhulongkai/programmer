...
//            sb.append("private ").append(TypeUtil.parseTypeFormSqlType(infos.get(i).getType())).append(" ").append(infos.get(i).getPropertyName()).append(";\n");
     ...
     * @return
     */
    public static String generateEntityProperties(String parentClassName, List<Columns> infos, String foreignKey) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < infos.size(); i++) {
            if (infos.get(i).isIndex()) {
                if (i != 0) {
                    sb.append("    ");
                }
                sb.append("private ").append(infos.get(i).getJavaType()).append(" ").append(infos.get(i).getName()).append("; \n");
            }
        }
        // 外键为父表实体引用
